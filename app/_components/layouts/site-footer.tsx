import Link from 'next/link'

const footerSections = {
  categories: [
    { href: '/politics', label: '정치' },
    { href: '/economy', label: '경제' },
    { href: '/sports', label: '스포츠' },
    { href: '/tech', label: '기술' },
  ],
  services: [
    { href: '/about', label: '회사소개' },
    { href: '/contact', label: '문의하기' },
    { href: '/privacy', label: '개인정보처리방침' },
    { href: '/terms', label: '이용약관' },
  ],
}

const socialLinks = [
  {
    href: '#',
    label: 'Twitter',
    icon: (
      <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
    ),
  },
  {
    href: '#',
    label: 'Facebook',
    icon: (
      <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
    ),
  },
]

export default function SiteFooter() {
  return (
    <footer className="bg-gray-50 border-t mt-auto">
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Brand Section */}
          <div>
            <h3 className="font-semibold text-gray-900 mb-4">Beom News</h3>
            <p className="text-gray-600 text-sm">
              신뢰할 수 있는 뉴스와 정보를 제공하는 플랫폼입니다.
            </p>
          </div>
          
          {/* Categories */}
          <div>
            <h4 className="font-semibold text-gray-900 mb-4">카테고리</h4>
            <ul className="space-y-2 text-sm">
              {footerSections.categories.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="text-gray-600 hover:text-gray-900">
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
          
          {/* Services */}
          <div>
            <h4 className="font-semibold text-gray-900 mb-4">서비스</h4>
            <ul className="space-y-2 text-sm">
              {footerSections.services.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="text-gray-600 hover:text-gray-900">
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
          
          {/* Social Links */}
          <div>
            <h4 className="font-semibold text-gray-900 mb-4">팔로우</h4>
            <div className="flex space-x-4">
              {socialLinks.map(({ href, label, icon }) => (
                <a 
                  key={label}
                  href={href} 
                  className="text-gray-400 hover:text-gray-600"
                  aria-label={label}
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    {icon}
                  </svg>
                </a>
              ))}
            </div>
          </div>
        </div>
        
        <div className="border-t pt-8 mt-8 text-center text-sm text-gray-600">
          <p>&copy; 2024 Beom News. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
}